...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are an expert education assessment AI. Generate a thought-provoking question about ${topic} at ${difficulty} difficulty level.

Guidelines:
- Easy: Basic concepts, definitions, simple examples
- Medium: Application of concepts, comparisons, analysis
- Hard: Complex scenarios, critical thinking, synthesis of multiple concepts
...
Previous questions to avoid repeating: ${previousQuestions.join(", ")}`;
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: `Generate a ${difficulty} difficulty question about ${topic}.` }
        ],
        temperature: 0.8,
      }),
    });
